...
import { getLeadership, getSiteSettings } from '../../../lib/cms'
import { resolvePlaceholderLabel, resolvePlaceholderMode, shouldShowPlaceholder } from '../../../lib/placeholders'

export const dynamic = 'force-dynamic'

export default async function LeadershipPage() {
  const [leadership, site] = await Promise.all([getLeadership(), getSiteSettings()])
  ...
  const showPlaceholder = shouldShowPlaceholder({
    mode,
    override: 'default',
    contentExists: leadership.length > 0,
  })
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-12">
        ...
        {showPlaceholder ? (
          <ContentPlaceholder
            ...
            label={label}
            description="Leadership profiles will appear here once published."
            items={2}
            columns={2}
          />
        ) : (
